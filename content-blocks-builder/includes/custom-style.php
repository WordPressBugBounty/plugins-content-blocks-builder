	...
	class CustomStyle extends CoreComponent {
		...
		public function build_block_link_to_post( $block_content, $block, $block_instance ) {
			...
			if ( 'image' === $media_type && ( $background['image']['useFeaturedImage'] ?? false ) && ( $background['image']['linkToPost'] ?? false ) ) {
				$block_content = $this->add_inner_content_to_block( $block_content, '<a class="bb:link-to-post" href="' . esc_url( get_permalink( $post_id ) ) . '" aria-label="' . esc_attr( get_the_title( $post_id ) ) . '" rel="permalink"></a>' );
			}
		...
		private function build_carousel_settings( $carousel_settings, $exclude_li_role = false ) {
			...
			if ( in_array($effect, $slidesPerViewDependencies, true ) ) {
				...
				if ( $slidesPerView ) {
					...
					if ( $slidesPerView >= 2 && $slidesPerGroup ) {
